...
const BlogPostDetail = ({ 
  ...
  const handleSocialShare = (platform, url, title) => {
    let shareUrl = '';
    ...
    switch (platform) {
      case 'facebook':
        ...
        break;
      case 'twitter':
        shareUrl = `https://twitter.com/share?text=${encodeURIComponent(title)}&url=${encodeURIComponent(url)}`;
        break;
      case 'linkedin':
  ...
  const blogUrl = `${baseUrl}/blog-detail/${blogData.slug}`;

  return (
    <article className="hentry post post-standard has-post-thumbnail post-standard-details">
      <div className="post-thumb">
        <Image 
          src={`${baseUrl}/public/blog_image/${blogData.image}`} 
          alt="post"
          width={800}
          height={400}
          priority
      ...
      <div className="post__content">
        <div className="post__date">
          <time className="published" dateTime="2017-03-20 12:00:00">
            ...
            <a className="number">{date}</a>
            <span className="month">{month}</span>
          </time>
        </div>

        <div className="post__content-info">
          <h3 className="post__title entry-title">{blogData.title}</h3>
          {/* <div className="post-additional-info"><span className="category">Under<a> WebSite</a></span></div> */}
        </div>
        ...
        <div className="post-details-shared">
          ...
          <div className="widget w-follow">
            <ul className="socials socials--round">
              ...
              <li>
                <button
                  onClick={() => handleSocialShare('facebook', blogUrl, blogData.title)}
              ...
              <li>
                <button
                  onClick={() => handleSocialShare('twitter', blogUrl, blogData.title)}
                  aria-label="twitter"
                  className="react-share__ShareButton"
                  style={{
                    backgroundColor: 'transparent',
              ...
              <li>
                <button
                  onClick={() => handleSocialShare('linkedin', blogUrl, blogData.title)}
                  aria-label="linkedin"
                  className="react-share__ShareButton"
                  style={{
                    backgroundColor: 'transparent',
              ...
              <li>
                <button
                  onClick={() => handleSocialShare('whatsapp', blogUrl, blogData.title)}
                  aria-label="whatsapp"
                  className="react-share__ShareButton"
                  style={{
                    backgroundColor: 'transparent',
...
<BlogPostDetail 
  ...
  month="Mar"
  blogTags={[
    { slug: 'tag-1', tag: 'Tag 1' },
    { slug: 'tag-2', tag: 'Tag 2' }
  ]}
