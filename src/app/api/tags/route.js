...
export async function GET(request) {
  try {
    ...
    let query = 'SELECT id, name, slug, created_at, updated_at FROM tags WHERE 1=1';
    const params = [];
    ...
    if (slug) {
      if (rows.length === 0) return NextResponse.json({ error: 'Tag not found' }, { status: 404 });
      return NextResponse.json({ tag: rows[0] }, { status: 200 });
    }
    ...
    console.error('Database Error (tags):', error);
